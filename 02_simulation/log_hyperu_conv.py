...
"""This script is implementing the alternative, convergin version of the log-hyperu penalty."""
...
"""
class log_hyperu_alt(torch.autograd.Function):
    @staticmethod
    def forward(ctx, a, b, x):

        u_res = robust_hyperu_conv(a, b, x)
        test = torch.tensor([[u_res.item() + 1]])
    ...
    def backward(ctx, grad_output):
        ...
        grad_x = grad_output * (-a * torch.div(robust_hyperu_conv(a + 1, b + 1, x), u_res))

        return None, None, grad_x
    
# Alias the apply method:
log_hyperu_conv = log_hyperu_alt.apply