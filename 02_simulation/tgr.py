...
if __name__ == "__main__":
    ...
    model = TripleGammaRegularization(input_dim=10, output_dim=1,lr=0.00001, bias=False)
    ...
    print(f'MSE Loss OLS: {np.square(np.subtract(y_test, torch.matmul(X_test,torch.matmul(torch.inverse(torch.matmul(torch.transpose(X_train, 0, 1), X_train)), torch.matmul(torch.transpose(X_train, 0, 1), y_train))))).mean()}')
